...
def test_x_dci_team_id_header_filter_view_sso(
    ...
    # sso_user2 don't have access to any products or teams
    ...
    sso_user2_id = sso_client_user4.get("/api/v1/identity").data["identity"]["id"]

    # admin add sso_user2 to team1 and team2
    add_sso_user2_to_team1 = client_admin.post(
        "/api/v1/teams/%s/users/%s" % (team1_id, sso_user2_id), data={}
    )
    assert add_sso_user2_to_team1.status_code == 201
    add_sso_user2_to_team2 = client_admin.post(
        "/api/v1/teams/%s/users/%s" % (team2_id, sso_user2_id), data={}
    )
    assert add_sso_user2_to_team2.status_code == 201

    # Now sso_user2 see products available to team1 and team2
    assert len(sso_client_user4.get("/api/v1/teams").data["teams"]) == 2
    assert len(sso_client_user4.get("/api/v1/products").data["products"]) == 2

    # If we scope the view to team1, then sso_user2 keep the view
    assert (
        len(
    ...
    # If we scope the view to team2, then sso_user2 lost the view
    assert (
        len(
            sso_client_user4.get(
