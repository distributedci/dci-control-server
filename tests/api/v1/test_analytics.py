...
def test_handle_es_sort():
    ...
    assert res == [{"created_at": {"order": "desc", "format": "strict_date_optional_time"}}]


def test_handle_es_timeframe():
    query = qed.build("name=titi")
    res = analytics.handle_es_timeframe(query, {"from": "2024-01-01", "to": "2024-02-01"})
    assert res == {
            "bool": {
                "filter": [
...
def test_handle_includes_excludes():
    ret = analytics.handle_includes_excludes({"includes": "titi,tata", "excludes": "toto"})
