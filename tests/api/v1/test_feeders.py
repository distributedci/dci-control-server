...
def test_success_get_feeder_authorized_users(client_admin, client_epm, feeder):
...
def test_failure_get_feeder_unauthorized_users(client_user1, feeder, team1_id):
...
def test_success_delete_feeder_authorized_users(
    client_admin, client_epm, feeder, team1_id
):
    ...
    admin_retrieve = client_admin.get("/api/v1/feeders/%s" % feeder["id"])
...
def test_failure_delete_feeder_unauthorized_users(client_user1, feeder):
...
def test_success_put_feeder_authorized_users(client_admin, client_epm, feeder):
    ...
    admin_result = client_admin.get("/api/v1/feeders/%s" % feeder["id"])
    ...
    epm_result = client_epm.get("/api/v1/feeders/%s" % feeder["id"])

    assert epm_result.data["feeder"]["name"] == "newname-po"


def test_failure_put_feeder_unauthorized_users(client_user1, feeder):
...
def test_success_refresh_secret_feeder_authorized_users(
    client_admin, client_epm, feeder
):
    """Test to ensure user with proper permissions can update feeders."""
...
def test_failure_refresh_secret_feeder_unauthorized_users(client_user1, feeder):
    """Test to ensure user w/o proper permissions can't update feeders."""
...
def test_success_ensure_put_api_secret_is_not_leaked(client_admin, feeder):
