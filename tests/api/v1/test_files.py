...
def test_create_files(client_user1, jobstate_user_id):
    file_id = t_utils.create_task_file(
        client_user1, jobstate_user_id, "file", "content"
    )["id"]
...
def test_upload_tests_with_regressions_successfix(
    mocked_disp, client_admin, hmac_client_team1, rhel_80_topic, rhel_80_component
):
    headers = {
...
def test_create_file_as_user(client_user1, jobstate_user_id):
...
def test_get_file_as_user(client_user1, file_user_id, jobstate_user_id):
    file = client_user1.get("/api/v1/files/%s" % file_user_id)
    assert file.status_code == 200


def test_delete_file_as_user(client_user1, file_user_id):
    file_delete = client_user1.delete("/api/v1/files/%s" % file_user_id)
    assert file_delete.status_code == 204


def test_get_file_content_as_user(client_user1, jobstate_user_id):
    content = "azertyuiop1234567890"
    file_id = t_utils.create_task_file(client_user1, jobstate_user_id, "foo", content)[
        "id"
    ]
...
def test_change_file_to_invalid_state(client_admin, file_user_id):
    t = client_admin.get("/api/v1/files/" + file_user_id).data["file"]
    data = {"state": "file"}
    r = client_admin.put(
        "/api/v1/files/" + file_user_id, data=data, headers={"If-match": t["etag"]}
    )
    assert r.status_code == 405
    current_file = client_admin.get("/api/v1/files/" + file_user_id)
