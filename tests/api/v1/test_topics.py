...
def test_subscription(admin, user, user_id, topic_user_id):
    r = admin.get("/api/v1/topics/%s/users" % topic_user_id)
    assert len(r.data["users"]) == 0

    r = user.post("/api/v1/topics/%s/users" % topic_user_id)
    assert r.status_code == 201

    r = admin.get("/api/v1/topics/%s/users" % topic_user_id)
    assert r.data["users"][0]["id"] == user_id

    r = user.delete("/api/v1/topics/%s/users" % topic_user_id)
