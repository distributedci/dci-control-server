...
class S3(stores.Store):
    ...
    def delete(self, filename):
        try:
            logger.info(f"Deleting file: {self.bucket} -> {filename}")
            self.s3.delete_object(Bucket=self.bucket, Key=filename)
            logger.info(f"✓ Deleted file: {self.bucket} -> {filename}")
        except ClientError as e:
            if int(e.response["Error"]["Code"]) == 404:
                logger.warning(f"file '{filename}' not found in s3 bucket '{self.bucket}'")
                return
