...
def _generate_es_query(parsed_query, handle_nested=True):
    if len(parsed_query) > 0 and isinstance(parsed_query, list) and isinstance(parsed_query[0], str):
        return _generate_from_operators(parsed_query, handle_nested)
    if len(parsed_query) == 1 and isinstance(parsed_query, list) and isinstance(parsed_query[0], list):
