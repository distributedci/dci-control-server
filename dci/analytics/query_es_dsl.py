...
query << (
    (
        ...
        + pp.Group(query)
        + _rp
        + pp.ZeroOrMore(_logical_operators + pp.Group(query))
    )
    | _logical_operation
)
...
def _generate_from_comparison_operators(parsed_query):
...
def _generate_es_query(parsed_query):
    if (
        ...
        return _generate_from_comparison_operators(parsed_query)
    elif (
        isinstance(parsed_query[0], list)
        and len(parsed_query) == 1
        and isinstance(parsed_query[0][0], str)
    ):
        return _generate_from_comparison_operators(parsed_query[0])
    else:
                ...
                return {
                    "nested": {
                        ...
                        "query": {
                            "bool": {
                                "should": [
                                    _generate_es_query(left_operands),
            ...
            if path:
                return {
                    "nested": {
                        ...
                        "query": {
                            "bool": {
                                "filter": [_generate_es_query(o) for o in operands]
                            }
                        },
                    }
                }
