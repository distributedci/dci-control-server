...
def _eq(args, model_object, sql, columns):
    logger.info(f"eq {args}")
    if len(args) != 2:
        raise SyntaxError(f"invalid number of args {len(args)} for eq")
    col = check_column(execute(args[0], model_object, sql, columns), columns)
    val = execute(args[1], model_object, sql, columns)
    m_column = getattr(model_object, col)
    logger.info(f"{col} == {val}")
    return sql.filter(m_column == val)


def _ne(args, model_object, sql, columns):
    logger.info(f"ne {args}")
    if len(args) != 2:
...
def _like(args, model_object, sql, columns):
    logger.info(f"like {args}")
    if len(args) != 2:
    ...
    return sql.filter(m_column.like(execute(args[1], model_object, sql, columns)))


def _ilike(args, model_object, sql, columns):
    logger.info(f"ilike {args}")
    if len(args) != 2:
    ...
    return sql.filter(m_column.ilike(execute(args[1], model_object, sql, columns)))


def _in(args, model_object, sql, columns):
    logger.info(f"in {args}")
    if len(args) != 2:
        raise SyntaxError(f"invalid number of args {len(args)} for in")
    col_name = execute(args[0], model_object, sql, columns)
    m_column = getattr(
        model_object,
        check_column(col_name, columns),
    )
    if isinstance(m_column.type, ARRAY):
        return sql.filter(
            m_column.contains([execute(args[1], model_object, sql, columns)])
        )
    raise SyntaxError(f"{col_name} is not an array")


def _and(args, model_object, sql, columns):
    logger.info(f"and {args}")
    return sql.filter(and_(*[execute(a, model_object, sql, columns) for a in args]))


def _or(args, model_object, sql, columns):
    logger.info(f"or {args}")
    return sql.filter(or_(*[execute(a, model_object, sql, columns) for a in args]))
