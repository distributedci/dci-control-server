...
def _q(args, model_object, sql, columns, do_filter):
    ...
    return build(args[0], model_object, sql, columns)
...
def _contains(args, model_object, sql, columns, do_filter):
    log(f"contains {args}")
    if len(args) < 2:
...
def _and(args, model_object, sql, columns, do_filter):
    log(f"and {args}")
    for a in args:
        sql = build(a, model_object, sql, columns, do_filter)
    return sql
...
def _or(args, model_object, sql, columns, do_filter):
    ...
    clauses = []
    for a in args:
        clauses.append(build(a, model_object, sql, columns, False))
