...
A new component has been created: {component}
""".format(
        topic=event["topic_name"], component=event["component_name"]
    )
...
def get_emails_from_topic(topic_id):
    try:
        ...
        query = query.options(sa_orm.joinedload("topic", innerjoin=True)).options(
            sa_orm.joinedload("user", innerjoin=True)
        )

        return [ut.user.email for ut in query.all()]
    except dci_exc.DCIException:
        return []
...
def get_component_event(component, emails):
    ...
    return {
        ...
        "topic_name": str(component["topic_name"]),
        "status": "new",
    }
...
def dispatcher(job=None, component=None):
    if job:
