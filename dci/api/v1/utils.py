...
def get_columns_name_with_objects(table, table_prefix=False):
    if table_prefix:
        columns = {
            "%s_%s"
            % (table.name, column.name): getattr(table.columns, column.name)  # noqa
...
class QueryBuilder(object):
    ...
    def _get_sort_query_with_embeds(
        ...
        if root_table_name in embeds.EMBED_STRING_TO_OBJECT:
            for embed_elem in embeds.EMBED_STRING_TO_OBJECT[
                root_table_name
            ].values():  # noqa
        ...
        return sort_query(
            args_sort, strings_to_columns, strings_to_columns_with_embeds
        )  # noqa
    ...
    def get_query(self, use_labels=True):
        ...
        query = sql.select(
            select_clause, use_labels=use_labels, from_obj=children
        )  # noqa
        if self._embeds:
            ...
            query = sql.select(
                select_clause, use_labels=True, from_obj=children
            )  # noqa
