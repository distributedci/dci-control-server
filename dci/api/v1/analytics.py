    ...
    try:
        ...
        if res.status_code == 200:
            ...
            raise dci_exc.DCIException(
                f"resource not found in backend server: {res.text}", status_code=404
            )
        else:
            logger.error(f"analytics error: {res.text}")
            raise dci_exc.DCIException(
                f"error with backend service: {res.text}", status_code=res.status_code
            )
    except ConnectionError as e:
        logger.error(f"analytics connection error: {e}")
        raise dci_exc.DCIException(
