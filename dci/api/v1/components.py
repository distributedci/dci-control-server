...
def create_components(user):
    ...
    values["display_name"] = values.get(
        "display_name"
    ) or migration_components.get_component_display_name(name, canonical_project_name)
    values["version"] = values.get(
        "version"
    ) or migration_components.get_component_version(name, canonical_project_name)
