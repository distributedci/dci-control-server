...
def schedule_jobs(user):
    ...
    if inputs.get("pipeline_name"):
        pipeline_name = inputs.pop("pipeline_name")
        pipeline_inputs = {
            "name": pipeline_name,
            "topic_id": topic_id,
            "team_id": remoteci.team_id,
        }
        pipeline_inputs.update(v1_utils.common_values_dict())
        p = base.create_resource_orm(models2.Pipeline, pipeline_inputs)
        inputs["pipeline_id"] = p["id"]
