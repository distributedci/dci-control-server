...
from dci.common.schemas import check_json_is_valid, create_pipeline_schema, check_and_get_args
...
def get_pipeline_by_id(user, p_id):
    p = base.get_resource_orm(
        ...
        p_id,
        options=[
            sa_orm.selectinload("topic"),
            sa_orm.selectinload("team")
        ],
    )
...
def get_pipelines(user):
    ...
    query = (
        query.options(sa_orm.joinedload("topic", innerjoin=True))
...
def get_jobs_from_pipeline(user, p_id):
    ...
    query = query.order_by(models2.Job.created_at.desc())
    query = (
