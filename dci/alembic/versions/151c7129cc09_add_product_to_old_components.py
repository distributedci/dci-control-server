...
def upgrade():
    ...
    try:
        ...
        limit = 3000
        while True:
            query = s.query(models2.Component).offset(offset).limit(limit)
            ...
            for c in components:
                if c.topic_id not in _topic_to_product:
                    ...
                    p = (
                        ...
                        .filter(models2.Product.id == t.product_id)
                        .one()
                    )
                    _topic_to_product[c.topic_id] = p
                p = _topic_to_product[c.topic_id]
